...
export default function SubscribePage() {
  ...
  // Extract active subscription info when status loads
  useEffect(() => {
    if (subscriptionStatus?.isActive && subscriptionStatus.subscriptionTier && subscriptionStatus.endDate) {
      setActiveSubscriptionInfo({
        tier: subscriptionStatus.subscriptionTier,
        endDate: formatDate(subscriptionStatus.endDate),
      });
    } else {
      ...
      {isVerifying ? (
        ...
        <div className="max-w-md mx-auto p-8 border border-amber-100 bg-amber-50 rounded-lg shadow-md">
          ...
          <p className="text-gray-700 mb-6">
            You have a subscription that's waiting for payment completion. Would you like to complete your payment now?
          </p>
          <button
            onClick={() => {
              queryClient.invalidateQueries({ queryKey: ['subscriptionStatus'] })
